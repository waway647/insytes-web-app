...
<div class="w-full flex flex-col gap-6 font-sans antialiased text-slate-200">
    ...
    <div class="w-full flex flex-col lg:flex-row gap-6 p-4">
        <div class="w-full lg:w-1/2 flex flex-col gap-4 p-4 rounded-2xl bg-gradient-to-b from-[#0f0f10] to-[#0b0b0b] border border-white/5 shadow hover:shadow-lg transition">
            <p class="text-sm text-white font-semibold">Strengths</p>
            <div class="w-full flex flex-col md:flex-row gap-4">
                <?php if (!empty($strength)): ?>
                    <?php foreach ($strength as $s): ?>
                        <span class="flex-1 px-3 py-2 bg-white/5 rounded-lg text-sm text-center text-slate-300 break-words">
    ...
    <div class="w-full flex flex-col gap-4 p-4">
        ...
        <div class="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            $metrics_map = [
                ...
                'attacking_third_possession' => 'Attacking Third Possession',
                'corners_awarded' => 'Corners Awarded',
                'offsides' => 'Offsides'
            ];
            foreach ($metrics_map as $k => $label):
                $val = $key_metrics[$k] ?? '';
            ?>
            <div class="w-full p-5 bg-[#1D1D1D] rounded-xl flex flex-col items-center gap-2 border border-white/5 shadow-sm hover:shadow-md transition transform hover:-translate-y-1">
                <?php if (in_array($k, ['duels_won_percentage','possession_percentage','attacking_third_possession'])): ?>
    ...
    <div class="w-full flex flex-col gap-4 p-4">
        ...
        <div class="w-full px-6 py-6 bg-gradient-to-b from-[#0f0f10] to-[#0b0b0b] rounded-2xl border border-white/5 shadow hover:shadow-lg transition flex flex-col gap-4">

            <div class="w-full flex flex-col divide-y divide-white/5">
                <div class="py-4 flex flex-col md:flex-row md:gap-4 md:items-start">
                ...
                <div class="py-4 flex flex-col md:flex-row md:gap-4">
